...
2. **软键盘行为**：LLM 对话输入框 focus 时弹出模拟键盘面板（仅原型演示用），定案形态 = **输入区紧贴键盘上沿，两者之间只有 composer 自身 padding、零多余空白**；对话列表同步收缩不遮挡。Compose 侧对应 `imePadding` 正确实现，切勿在输入区与键盘间叠加额外 spacing。
