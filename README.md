...
![安卓娘演示：四步改原型](docs/comic-ai-agent.webp)

> **不用会编程，也能改原型**（漫画图解）：① 先想清楚哪里不好看、哪里不好用 → ② 把想法用大白话说给 AI 助手（「把分类改成侧边栏」）→ ③ 改完自己走查：明暗主题、横竖屏都看一遍 → ④ 提交改动，被采纳就进正式 App。
>
> 插图为品牌素材（同 logo、图标、MoeApk 字体），**不适用 MIT**，见 [`assets/LICENSE.txt`](assets/LICENSE.txt)。
